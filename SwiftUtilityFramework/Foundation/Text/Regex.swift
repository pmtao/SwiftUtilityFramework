...
//
//  Created by Meler Paine on 18-10-22.
//  Copyright © 2019年 SinkingSoul. All rights reserved.
//

import Foundation

// 参考: http://www.hangge.com/blog/cache/detail_2170.html
...
public struct Regex {
    ...
    //使用正则表达式进行初始化
    public init(_ pattern: String, options: RegexMatchOptions = []) throws {
...
//正则匹配可选项
extension Regex {
    /// RegexMatchOptions 定义了正则表达式匹配时的行为
    public struct RegexMatchOptions: OptionSet {
        
        //忽略字母
        public static let ignoreCase = RegexMatchOptions(rawValue: 1)
        
        //忽略元字符
        public static let ignoreMetacharacters = RegexMatchOptions(rawValue: 1 << 1)
        
        //默认情况下,“^”匹配字符串的开始和结束的“$”匹配字符串,无视任何换行。
        //使用这个配置，“^”将匹配的每一行的开始,和“$”将匹配的每一行的结束。
        public static let anchorsMatchLines = RegexMatchOptions(rawValue: 1 << 2)
        
        ///默认情况下,"."匹配除换行符(\n)之外的所有字符。使用这个配置，选项将允许“.”匹配换行符
        public static let dotMatchesLineSeparators = RegexMatchOptions(rawValue: 1 << 3)
        
        //OptionSet的 raw value
        public let rawValue: Int
        
        //将Regex.RegexMatchOptions 转换成对应的 NSRegularExpression.Options
...
extension Regex {
    ...
    static func getAllMatchString(_ content: String,
                                  pattern: String,
                                  options: RegexMatchOptions = []) -> [String] {
        var results = [String]()
